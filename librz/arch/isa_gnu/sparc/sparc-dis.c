...
int
print_insn_sparc (bfd_vma memaddr, disassemble_info *info, void *data)
{
	  ...
	  && (opcode->lose & insn) == 0)
	{
...

	  {
		...
		while (*s == ',')
		  {
		    ...
		    switch (*s)
		      {
		      case 'a':
			(*info->fprintf_func) (stream, data, "a");
			is_annulled = 1;
			++s;
			continue;
		      case 'N':
			(*info->fprintf_func) (stream, data, "pn");
			++s;
			continue;

		      case 'T':
			(*info->fprintf_func) (stream, data, "pt");
			++s;
			continue;
		...
		switch (*s)
		  ...
		  case '#':
		    (*info->fprintf_func) (stream, data, "0");
		    break;
...
#define	freg(n, data)		(*info->fprintf_func) (stream, data, "%%%s", freg_names[n])
#define	fregx(n, data)	(*info->fprintf_func) (stream, data, "%%%s", freg_names[((n) & ~1) | (((n) & 1) << 5)])
		  case 'e':
		    freg (X_RS1 (insn), data);
		    break;
		  case 'v':	/* Double/even.  */
		  case 'V':	/* Quad/multiple of 4.  */
		    fregx (X_RS1 (insn), data);
		    break;

		  case 'f':
		    freg (X_RS2 (insn), data);
		    break;
		  case 'B':	/* Double/even.  */
		  case 'R':	/* Quad/multiple of 4.  */
		    fregx (X_RS2 (insn), data);
		    break;
		  ...
		  case 'J':	/* Quad/multiple of 4.  */
		    fregx (X_RD (insn), data);
		    break;
#undef	freg
#undef	fregx

#define	creg(n, data)	(*info->fprintf_func) (stream, data, "%%c%u", (unsigned int) (n))
		  case 'b':
		    creg (X_RS1 (insn), data);
		    break;

		  case 'c':
		    creg (X_RS2 (insn), data);
		    break;
...
#undef	creg
		  ...
		  case 'h':
		    (*info->fprintf_func) (stream, data, "%%hi(%#x)",
					   ((unsigned) 0xFFFFFFFF
					    & ((int) X_IMM22 (insn) << 10)));
		    break;
		  ...
		  case 'j':	/* 10 bit immediate.  */
		    {
			 ...
			 not before it.  */
		      ...
		      if (imm <= 9) {
			      (*info->fprintf_func) (stream, data, "%d", imm);
		      } else {
			      (*info->fprintf_func) (stream, data, "%#x", imm);
		      }
		    }
		    break;
		  ...
		  case 'Y':	/* 6 bit unsigned immediate.  */
		    {
		      ...
		      if (imm <= 9) {
			      ...
			      (info->fprintf_func) (stream, data, "%#x", (unsigned)imm);
		      }
		    }
		    break;

		  case '3':
		    (info->fprintf_func) (stream, data, "%ld", X_IMM (insn, 3));
		    break;
		      ...
		      if (mask == 0) {
			      ...
			      while (bit) {
				      if (mask & bit) {
					      ...
					      (info->fprintf_func) (stream, data, "%s", name);
					      printed_one = 1;
				      }
				      bit >>= 1;
			      }
		  ...
		  case '8':
		  case '9':
		    (*info->fprintf_func) (stream, data, "%%fcc%c", *s - '6' + '0');
		    break;

		  case 'z':
		    (*info->fprintf_func) (stream, data, "%%icc");
		    break;

		  case 'Z':
		    (*info->fprintf_func) (stream, data, "%%xcc");
		    break;

		  case 'E':
		    (*info->fprintf_func) (stream, data, "%%ccr");
		    break;

		  case 's':
		    (*info->fprintf_func) (stream, data, "%%fprs");
		    break;

		  case 'o':
		    (*info->fprintf_func) (stream, data, "%%asi");
		    break;

		  case 'W':
		    (*info->fprintf_func) (stream, data, "%%tick");
		    break;
		  ...
		  case '?':
			  if (X_RS1 (insn) == 31) {
				  (*info->fprintf_func) (stream, data, "%%ver");
			  } else if ((unsigned)X_RS1 (insn) < 17) {
				  (*info->fprintf_func) (stream, data, "%%%s",
					  v9_priv_reg_names[X_RS1 (insn)]);
			  } else {
				  (*info->fprintf_func) (stream, data, "%%reserved");
			  }
			  break;
		  ...
		  case '*':
		    {
		      ...
		      if (name) {
			      ...
			      (*info->fprintf_func) (stream, data, "%ld", X_RD (insn));
		      }
		      break;
		    }

		  case 'M':
		    (*info->fprintf_func) (stream, data, "%%asr%ld", X_RS1 (insn));
		    break;

		  case 'm':
		    (*info->fprintf_func) (stream, data, "%%asr%ld", X_RD (insn));
		    break;
		  ...
		  case 'n':
		    (*info->fprintf_func)
		      (stream, data, "%#x", SIGN_EXT (X_DISP22 (insn), 22));
		    break;
		  ...
		  case 'A':
		    {
		      ...
		      if (name) {
			      ...
			      (*info->fprintf_func) (stream, data, "(%ld)", X_ASI (insn));
		      }
		      break;
		    }

		  case 'C':
		    (*info->fprintf_func) (stream, data, "%%csr");
		    break;

		  case 'F':
		    (*info->fprintf_func) (stream, data, "%%fsr");
		    break;

		  case 'p':
		    (*info->fprintf_func) (stream, data, "%%psr");
		    break;

		  case 'q':
		    (*info->fprintf_func) (stream, data, "%%fq");
		    break;

		  case 'Q':
		    (*info->fprintf_func) (stream, data, "%%cq");
		    break;

		  case 't':
		    (*info->fprintf_func) (stream, data, "%%tbr");
		    break;

		  case 'w':
		    (*info->fprintf_func) (stream, data, "%%wim");
		    break;

		  case 'x':
		    (*info->fprintf_func) (stream, data, "%ld",
					   ((X_LDST_I (insn) << 8)
					    + X_ASI (insn)));
		    break;

		  case 'y':
		    (*info->fprintf_func) (stream, data, "%%y");
		    break;
		  ...
		  case 'U':
		    {
		      ...
		      if (name) {
			      ...
			      (*info->fprintf_func) (stream, data, "%%cpreg(%d)", val);
		      }
		      break;
		    }
		  }
  ...
  (*info->fprintf_func) (stream, data, _("unknown"));
  return sizeof (buffer);
}
